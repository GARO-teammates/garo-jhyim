...
class Pi05InferenceGUI:
    def __init__(self):
        ...
        self.baseline_file = os.path.join(_THIS_DIR, "rx1_baseline_v5.json")

        # ========== 디버그 로그 파일 ==========
        self.debug_log_dir = os.path.join(_THIS_DIR, "debug_logs")
        ...
        # Pi0.5 카메라 매핑 (학습 데이터와 동일한 순서!)
        self.camera_names = ['top', 'wrist_right', 'wrist_left']
        ...
        # 수정: min/max 대신 실제 q01/q99 값 사용 (Pi0.5 QUANTILES 정규화)
        self.state_q01 = np.array([
            -0.32421875, -0.31787109375, 0.46484375, -0.7607421875, 0.14990234375, -0.12109375, 0.30859375, -1.0,
            -0.95361328125, -0.19677734375, -1.0, -0.28369140625, 0.08642578125, -0.107421875, -0.294921875, -0.794921875
        ], dtype=np.float32)
        self.state_q99 = np.array([
            0.953125, -0.1640625, 0.74755859375, -0.52490234375, 0.37939453125, 0.16552734375, 0.30859375, 0.7861328125,
            0.69873046875, 0.53271484375, -0.2265625, 0.99365234375, 0.423828125, 0.19384765625, 0.154296875, 0.99951171875
        ], dtype=np.float32)
        self.action_q01 = np.array([
            -0.32421875, -0.31787109375, 0.46484375, -0.7607421875, 0.14990234375, -0.12109375, 0.30859375, -1.0,
            -0.95361328125, -0.19677734375, -1.0, -0.28369140625, 0.08642578125, -0.107421875, -0.294921875, -0.794921875
        ], dtype=np.float32)
        self.action_q99 = np.array([
            0.953125, -0.1640625, 0.74755859375, -0.52490234375, 0.37939453125, 0.16552734375, 0.30859375, 0.7861328125,
            0.69873046875, 0.53271484375, -0.2265625, 0.99365234375, 0.423828125, 0.19189453125, 0.154296875, 0.99951171875
        ], dtype=np.float32)

        # Action Chunking 설정 (True: 매번 새 예측, False: 청크 재사용)
        self.force_new_prediction = False  # 수정: 기본값 False로 변경 (Action Chunking 활성화)

        # 비동기 추론 관련
        self.async_inference_enabled = True  # 비동기 추론 활성화
        self.next_actions = None  # 미리 예측된 다음 액션들
        self.inference_lock = threading.Lock()
        self.is_predicting = False  # 현재 예측 중인지
    ...
    def scan_models(self):
        ...
        지원 구조:
        1. 기존: v10/model.safetensors (flat)
        2. 신규: v20/010000/pretrained_model/model.safetensors (nested)
        """
        self.available_models = []
        self.model_paths = {}  # 모델명 -> 실제 경로 매핑

        if os.path.exists(self.models_dir):
            for version in os.listdir(self.models_dir):
                version_path = os.path.join(self.models_dir, version)
                if not os.path.isdir(version_path):
                    continue

                # 구조 1: 기존 flat 구조 (v10/, v11/)
                config_flat = os.path.join(version_path, "config.json")
                model_flat = os.path.join(version_path, "model.safetensors")
                if os.path.exists(config_flat) and os.path.exists(model_flat):
                    self.available_models.append(version)
                    self.model_paths[version] = version_path
                    continue

                # 구조 2: 신규 nested 구조 (v20/010000/pretrained_model/)
                for checkpoint in os.listdir(version_path):
                    checkpoint_path = os.path.join(version_path, checkpoint)
                    if not os.path.isdir(checkpoint_path):
                        continue

                    pretrained_path = os.path.join(checkpoint_path, "pretrained_model")
                    config_nested = os.path.join(pretrained_path, "config.json")
                    model_nested = os.path.join(pretrained_path, "model.safetensors")

                    if os.path.exists(config_nested) and os.path.exists(model_nested):
                        model_name = f"{version}/{checkpoint}"
                        self.available_models.append(model_name)
                        self.model_paths[model_name] = pretrained_path
    ...
    def create_ui(self):
        ...
        self.command_entry.insert(0, "Put the red snack in the box")
    ...
    def _init_debug_log(self):
        ...
        try:
            with open(self.debug_log_file, 'w') as f:
                ...
                f.write(f"{'='*80}\n\n")
                f.write(f"Joint ID Mapping:\n")
                f.write(f"  Right Arm: {self.joint_ids[:8]} (indices 0-7)\n")
                f.write(f"  Left Arm:  {self.joint_ids[8:]} (indices 8-15)\n\n")
            print(f"[DEBUG] Log file: {self.debug_log_file}")
        except Exception as e:
            print(f"[ERROR] Failed to init debug log: {e}")

    def _write_debug_log(self, message, section=False):
        """디버그 로그 파일에 기록"""
        try:
    ...
    def _write_debug_section(self, title):
        """디버그 로그에 섹션 구분선 기록"""
        try:
            with open(self.debug_log_file, 'a') as f:
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
    ...
    def _write_joint_debug(self, frame_num, current_positions, state_normalized, action_output, action_positions):
        """관절 정보 상세 디버그 로그"""
        try:
            with open(self.debug_log_file, 'a') as f:
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                arm_swap = getattr(self, 'arm_swap_mode', False)
                norm_type = getattr(self, 'state_norm_type', 'QUANTILES')

                f.write(f"\n[{timestamp}] ══════ Frame #{frame_num} ══════\n")
                f.write(f"arm_swap_mode={arm_swap}, norm_type={norm_type}\n\n")

                # 왼팔 관절 상세 정보 (학습된 팔)
                f.write(f"【 LEFT ARM (학습된 팔) 】\n")
                f.write(f"{'Joint':<8} {'Raw Pos':<10} {'Linear':<10} {'Normalized':<12} {'Action Out':<12} {'Cmd Pos':<10}\n")
                f.write(f"{'-'*70}\n")

                left_joint_ids = self.joint_ids[8:16]  # [21,22,23,24,25,26,27,31]
                for i, jid in enumerate(left_joint_ids):
                    raw_pos = current_positions.get(jid, 0)
                    linear = (raw_pos - 2048) / 2048.0

                    # arm_swap_mode면 state[0-7]에 왼팔 데이터
                    state_idx = i if arm_swap else i + 8
                    norm_val = state_normalized[state_idx] if state_idx < len(state_normalized) else 0

                    # action도 arm_swap_mode면 [0-7]이 왼팔용
                    action_idx = i if arm_swap else i + 8
                    action_val = action_output[action_idx] if action_idx < len(action_output) else 0

                    cmd_pos = action_positions[i + 8] if i + 8 < len(action_positions) else 0

                    f.write(f"J{jid:<7} {raw_pos:<10} {linear:<10.4f} {norm_val:<12.4f} {action_val:<12.4f} {cmd_pos:<10}\n")

                # 학습 통계와 비교
                f.write(f"\n【 학습 통계 비교 (LEFT ARM) 】\n")
                if norm_type == 'MEAN_STD' and hasattr(self, 'state_mean'):
                    f.write(f"{'Index':<8} {'Mean':<10} {'Std':<10} {'Current':<10} {'Diff':<10} {'Norm Diff':<12}\n")
                    f.write(f"{'-'*60}\n")
                    for i in range(8):
                        mean_val = self.state_mean[i]
                        std_val = self.state_std[i]
                        jid = left_joint_ids[i]
                        raw_pos = current_positions.get(jid, 0)
                        linear = (raw_pos - 2048) / 2048.0
                        diff = linear - mean_val
                        norm_diff = diff / std_val if std_val > 1e-6 else 0
                        f.write(f"{i:<8} {mean_val:<10.4f} {std_val:<10.4f} {linear:<10.4f} {diff:<10.4f} {norm_diff:<12.4f}\n")

                # 베이스라인 비교
                f.write(f"\n【 베이스라인 비교 (LEFT ARM) 】\n")
                f.write(f"{'Joint':<8} {'Baseline':<10} {'Current':<10} {'Diff':<10}\n")
                f.write(f"{'-'*40}\n")
                for i, jid in enumerate(left_joint_ids):
                    baseline = self.baseline_positions.get(jid, 2048)
                    current = current_positions.get(jid, 2048)
                    diff = current - baseline
                    f.write(f"J{jid:<7} {baseline:<10} {current:<10} {diff:<+10}\n")

                f.write(f"\n")
        except Exception as e:
            print(f"[ERROR] _write_joint_debug: {e}")

    def _write_model_info_to_debug(self, model_name):
        """모델 로드 정보를 디버그 로그에 기록"""
        try:
            with open(self.debug_log_file, 'a') as f:
                f.write(f"\n{'='*80}\n")
                f.write(f"  MODEL LOADED: {model_name}\n")
                f.write(f"{'='*80}\n\n")

                norm_type = getattr(self, 'state_norm_type', 'QUANTILES')
                arm_swap = getattr(self, 'arm_swap_mode', False)

                f.write(f"Normalization Type: STATE={norm_type}, ACTION={getattr(self, 'action_norm_type', 'QUANTILES')}\n")
                f.write(f"arm_swap_mode: {arm_swap}\n\n")

                # 학습 통계 기록
                if norm_type == 'MEAN_STD' and hasattr(self, 'state_mean'):
                    f.write(f"【 학습 State 통계 (MEAN_STD) 】\n")
                    f.write(f"{'Index':<8} {'Mean':<12} {'Std':<12}\n")
                    f.write(f"{'-'*35}\n")
                    for i in range(16):
                        f.write(f"{i:<8} {self.state_mean[i]:<12.6f} {self.state_std[i]:<12.6f}\n")
                    f.write(f"\n")

                    f.write(f"【 학습 Action 통계 (MEAN_STD) 】\n")
                    f.write(f"{'Index':<8} {'Mean':<12} {'Std':<12}\n")
                    f.write(f"{'-'*35}\n")
                    for i in range(16):
                        f.write(f"{i:<8} {self.action_mean[i]:<12.6f} {self.action_std[i]:<12.6f}\n")
                    f.write(f"\n")

                elif hasattr(self, 'state_q01'):
                    f.write(f"【 학습 State 통계 (QUANTILES) 】\n")
                    f.write(f"{'Index':<8} {'Q01':<12} {'Q99':<12}\n")
                    f.write(f"{'-'*35}\n")
                    for i in range(16):
                        f.write(f"{i:<8} {self.state_q01[i]:<12.6f} {self.state_q99[i]:<12.6f}\n")
                    f.write(f"\n")

                # 베이스라인 정보 기록
                f.write(f"【 현재 베이스라인 (LEFT ARM) 】\n")
                f.write(f"{'Joint':<8} {'Position':<10} {'Linear':<12}\n")
                f.write(f"{'-'*35}\n")
                left_joints = [21, 22, 23, 24, 25, 26, 27, 31]
                for i, jid in enumerate(left_joints):
                    pos = self.baseline_positions.get(jid, 2048)
                    linear = (pos - 2048) / 2048.0
                    f.write(f"J{jid:<7} {pos:<10} {linear:<12.4f}\n")
                f.write(f"\n")

                # 베이스라인과 학습 통계 비교
                if norm_type == 'MEAN_STD' and hasattr(self, 'state_mean'):
                    f.write(f"【 베이스라인 vs 학습 Mean 비교 (LEFT ARM) 】\n")
                    f.write(f"{'Index':<8} {'Baseline':<12} {'Train Mean':<12} {'Diff':<12} {'Norm Diff':<12}\n")
                    f.write(f"{'-'*60}\n")
                    for i, jid in enumerate(left_joints):
                        pos = self.baseline_positions.get(jid, 2048)
                        linear = (pos - 2048) / 2048.0
                        ...
                        diff = linear - mean_val
                        norm_diff = diff / std_val if std_val > 1e-6 else 0
                        warning = " ⚠️ OUT OF RANGE!" if abs(norm_diff) > 3 else ""
                        f.write(f"{i:<8} {linear:<12.4f} {mean_val:<12.4f} {diff:<+12.4f} {norm_diff:<+12.4f}{warning}\n")
                    f.write(f"\n")

                f.write(f"{'='*80}\n\n")
        except Exception as e:
            print(f"[ERROR] _write_model_info_to_debug: {e}")
    ...
    def load_model(self):
        ...
        # model_paths에서 실제 경로 가져오기 (신규 구조 지원)
        ...
        try:
            ...
            if hasattr(self, 'policy') and self.policy is not None:
                self.log("[모델] 이전 모델 해제 중...")
                del self.policy
                self.policy = None
                import gc
                gc.collect()
                torch.cuda.empty_cache()
                self.log("[모델] GPU 메모리 해제 완료")
            ...
            # 정규화 방식 확인 (QUANTILES vs MEAN_STD)
            norm_mapping = config_dict.get('normalization_mapping', {})
            self.state_norm_type = norm_mapping.get('STATE', 'QUANTILES')
            self.action_norm_type = norm_mapping.get('ACTION', 'QUANTILES')
            self.log(f"[정규화] STATE: {self.state_norm_type}, ACTION: {self.action_norm_type}")

            # 정규화 통계 로드 (stats.json 또는 preprocessor safetensors)
            stats_path = os.path.join(model_path, "stats.json")
            preprocessor_path = os.path.join(model_path, "policy_preprocessor_step_2_normalizer_processor.safetensors")
            ...
            if os.path.exists(stats_path):
                ...
                if 'observation.state' in stats:
                    ...
                    if self.state_norm_type == 'MEAN_STD':
                        self.state_mean = np.array(state_stats['mean'], dtype=np.float32)
                        self.state_std = np.array(state_stats['std'], dtype=np.float32)
                        self.log(f"[정규화] State MEAN_STD 로드됨 (stats.json)")
                    else:
                        self.state_q01 = np.array(state_stats['q01'], dtype=np.float32)
                        self.state_q99 = np.array(state_stats['q99'], dtype=np.float32)
                        self.log(f"[정규화] State QUANTILES 로드됨 (stats.json)")

                # Action 통계
                if 'action' in stats:
                    action_stats = stats['action']
                    if self.action_norm_type == 'MEAN_STD':
                        self.action_mean = np.array(action_stats['mean'], dtype=np.float32)
                        self.action_std = np.array(action_stats['std'], dtype=np.float32)
                        self.log(f"[정규화] Action MEAN_STD 로드됨 (stats.json)")
                    else:
                        self.action_q01 = np.array(action_stats['q01'], dtype=np.float32)
                        self.action_q99 = np.array(action_stats['q99'], dtype=np.float32)
                        self.log(f"[정규화] Action QUANTILES 로드됨 (stats.json)")

                    # 왼팔만 학습된 모델 감지 (indices 8-15가 zeros, 0-7에 데이터 있음)
                    action_std = np.array(action_stats.get('std', [0]*16), dtype=np.float32)
                    first_half_active = np.mean(action_std[:8]) > 0.01
                    second_half_active = np.mean(action_std[8:16]) > 0.01

                    if first_half_active and not second_half_active:
                        self.arm_swap_mode = True
                        self.log(f"[정규화] 왼팔 전용 모델 감지! (arm_swap_mode=True)")
                    else:
                        self.arm_swap_mode = False

            elif os.path.exists(preprocessor_path):
                # 신규 방식: preprocessor safetensors 파일
                pre_stats = load_file(preprocessor_path)

                # State 통계
                if self.state_norm_type == 'MEAN_STD':
                    self.state_mean = pre_stats['observation.state.mean'].numpy()
                    self.state_std = pre_stats['observation.state.std'].numpy()
                    self.log(f"[정규화] State MEAN_STD 로드됨 (preprocessor)")
                else:
                    self.state_q01 = pre_stats['observation.state.q01'].numpy()
                    self.state_q99 = pre_stats['observation.state.q99'].numpy()
                    self.log(f"[정규화] State QUANTILES 로드됨 (preprocessor)")

                # Action 통계
                if self.action_norm_type == 'MEAN_STD':
                    self.action_mean = pre_stats['action.mean'].numpy()
                    self.action_std = pre_stats['action.std'].numpy()
                    self.log(f"[정규화] Action MEAN_STD 로드됨 (preprocessor)")
                else:
                    self.action_q01 = pre_stats['action.q01'].numpy()
                    self.action_q99 = pre_stats['action.q99'].numpy()
                    self.log(f"[정규화] Action QUANTILES 로드됨 (preprocessor)")

                # 왼팔만 학습된 모델 감지
                action_std = pre_stats['action.std'].numpy()
                first_half_active = np.mean(action_std[:8]) > 0.01
                second_half_active = np.mean(action_std[8:16]) > 0.01
                ...
                self.log(f"[경고] 정규화 통계 없음 - 기본 QUANTILES 사용")
                self.arm_swap_mode = False

            # 디버그 로그에 모델 정보 기록
            self._write_model_info_to_debug(model_name)
    ...
    def _detect_cameras(self):
        ...
        safe_log(f"[카메라] 물리 카메라: Gemini {len(gemini_cams)}개 스트림, C270 {len(c270_cams)}개")
    ...
    def read_servo_position(self, servo_id):
        ...
        try:
            ...
            self.serial_port.timeout = 0.01  # 10ms timeout
    ...
    def read_all_positions(self):
        """모든 서보 현재 위치 읽기 (왼팔만 실제 읽기, 오른팔은 baseline)"""
        positions = {}
        # 오른팔 ID (고정 - baseline 사용)
        right_arm_ids = [11, 12, 13, 14, 15, 16, 17, 41]
        # 왼팔 ID (실제 읽기)
        left_arm_ids = [21, 22, 23, 24, 25, 26, 27, 31]

        # 오른팔은 baseline 사용 (읽기 생략)
        for joint_id in right_arm_ids:
            positions[joint_id] = self.baseline_positions.get(joint_id, 2048)

        # 왼팔만 실제 읽기
        for joint_id in left_arm_ids:
    ...
    def start_inference(self):
        ...
        # 추론 시작 정보를 디버그 로그에 기록
        self._write_inference_start_info()

        self.policy.reset()
        self.inference_thread = threading.Thread(target=self.inference_loop, daemon=True)
        self.inference_thread.start()

    def _write_inference_start_info(self):
        """추론 시작 정보를 디버그 로그에 기록"""
        try:
            with open(self.debug_log_file, 'a') as f:
                f.write(f"\n{'*'*80}\n")
                f.write(f"  INFERENCE STARTED: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"{'*'*80}\n\n")

                f.write(f"Language Command: {self.current_command}\n")
                f.write(f"FPS Setting: {self.inference_fps}\n")
                f.write(f"Force New Prediction: {self.force_new_prediction}\n\n")

                # 현재 로봇 관절 위치 기록
                current_pos = self.read_all_positions()
                f.write(f"【 추론 시작 시 관절 위치 (LEFT ARM) 】\n")
                f.write(f"{'Joint':<8} {'Current':<10} {'Linear':<12} {'Baseline':<10} {'Diff':<10}\n")
                f.write(f"{'-'*55}\n")
                left_joints = [21, 22, 23, 24, 25, 26, 27, 31]
                for jid in left_joints:
                    curr = current_pos.get(jid, 2048)
                    baseline = self.baseline_positions.get(jid, 2048)
                    linear = (curr - 2048) / 2048.0
                    diff = curr - baseline
                    f.write(f"J{jid:<7} {curr:<10} {linear:<12.4f} {baseline:<10} {diff:<+10}\n")
                f.write(f"\n")

                # 카메라 상태
                f.write(f"【 카메라 상태 】\n")
                for name in self.camera_names:
                    status = "OK" if name in self.cameras and self.cameras[name].isOpened() else "NOT CONNECTED"
                    f.write(f"  {name}: {status}\n")
                f.write(f"\n")
        except Exception as e:
            print(f"[ERROR] _write_inference_start_info: {e}")
    ...
    def stop_inference(self):
        ...
        # 추론 종료 정보를 디버그 로그에 기록
        self._write_debug_log(f"INFERENCE STOPPED at frame #{getattr(self, '_debug_frame_count', 0)}", section=True)
    ...
    def inference_loop(self):
        """추론 루프 - 원본 Pi0.5 방식 (Action Queue + 비동기 추론)"""
        interval = 1.0 / self.inference_fps
        inference_count = 0
        model_inference_count = 0  # 실제 모델 추론 횟수
        fps_history = []
        ...
        action_queue = deque(maxlen=50)  # n_action_steps = 50

        self.log(f"[추론] 원본 Pi0.5 방식 시작 (chunk_size=50, FPS={self.inference_fps})")
        ...
        while self.is_inferencing:
            ...
            # 2초마다 ACC=6 적용
            if time.time() - last_acc_time >= 2.0:
                self.apply_all_acceleration(6, silent=True)
                last_acc_time = time.time()

            try:
                # 큐가 비었을 때만 새 추론 (원본 방식)
                if len(action_queue) == 0:
                    # 관측값 준비
                    observation = self.prepare_observation(debug_log=False)

                    if observation is not None:
                        # 전체 액션 청크 예측 (50 스텝)
                        inference_start = time.time()
                        with torch.no_grad():
                            # predict_action_chunk 직접 호출 (50개 액션 한번에)
                            actions = self.policy.predict_action_chunk(observation)
                            # shape: [1, chunk_size, action_dim] -> [chunk_size, action_dim]
                        ...
                        self.log(f"[추론] 모델 #{model_inference_count}: {inference_time*1000:.0f}ms, 큐={len(action_queue)}개")
                ...
                if len(action_queue) > 0:
                    ...
                    # 액션 적용 (역정규화 + 서보 명령)
                    self.apply_action_from_chunk(action_values, self._debug_frame_count % 50 == 0)
                    ...
                    self.root.after(0, lambda f=current_fps, a=avg_fps, c=inference_count, q=queue_len:
                        self.status_labels['fps'].configure(
                            text=f"{f:.1f} (avg: {a:.1f}) Q:{q}"))
        ...
        if inference_count > 0:
            self.log(f"[성능] 프레임: {inference_count}회, 모델추론: {model_inference_count}회")
            self.log(f"[성능] 총 {total_time:.1f}초, 평균 {inference_count/total_time:.1f} FPS")
            self.log(f"[성능] 추론당 프레임: {inference_count/max(1,model_inference_count):.1f}개")

    def prepare_observation(self, debug_log=False):
        """관측값 준비"""
        try:
            observation = {}
            ...
            for i, joint_id in enumerate(self.joint_ids):
                # arm_swap_mode: 왼팔 데이터를 0-7에 넣어야 함
                if arm_swap:
                    if i < 8:
                        # state[0-7] ← 왼팔 joints (indices 8-15의 joint_id)
                        joint_id = self.joint_ids[i + 8]  # 왼팔 joint
                        stats_idx = i  # stats 0-7 사용
                    else:
                        # state[8-15] ← 오른팔 joints (indices 0-7의 joint_id)
                        joint_id = self.joint_ids[i - 8]  # 오른팔 joint
                        stats_idx = i  # stats 8-15 사용 (zeros)
                else:
                    stats_idx = i
                ...
                if norm_type == 'MEAN_STD' and hasattr(self, 'state_mean') and hasattr(self, 'state_std'):
                    # MEAN_STD 정규화: (x - mean) / std
                    std = self.state_std[stats_idx]
                    if std > 1e-6:
                        state[i] = (linear - self.state_mean[stats_idx]) / std
                    else:
                        state[i] = 0.0
                else:
                    # QUANTILES 정규화
                    range_val = self.state_q99[stats_idx] - self.state_q01[stats_idx]
                    if range_val > 0.001:
                        quantile = 2.0 * (linear - self.state_q01[stats_idx]) / range_val - 1.0
                    else:
                        quantile = 0.0
                    state[i] = np.clip(quantile, -1.0, 1.0)

            # 디버그: STATE 정규화 값 출력 (조건부)
            if debug_log:
                self.log(f"[DEBUG] State {norm_type} (R arm): {[f'{v:.3f}' for v in state[:8]]}")
                self.log(f"[DEBUG] State {norm_type} (L arm): {[f'{v:.3f}' for v in state[8:16]]}")

            # 디버그용 데이터 저장 (apply_action에서 사용)
            self._last_current_positions = current_positions
            self._last_state_normalized = state.copy()
    ...
    def apply_action_from_chunk(self, action_values, debug_log=False):
        """청크에서 꺼낸 단일 액션 적용 (원본 Pi0.5 방식)"""
        try:
            if action_values is None:
                return

            # action_values는 이미 numpy array [16]
            if len(action_values) < 16:
                self.log(f"[경고] 액션 차원 부족: {len(action_values)}")
                return

            # arm_swap_mode: 왼팔 전용 모델 처리
            arm_swap = getattr(self, 'arm_swap_mode', False)
            norm_type = getattr(self, 'action_norm_type', 'QUANTILES')
            ...
            for i, joint_id in enumerate(self.joint_ids):
                if i < len(action_values):
                    # arm_swap_mode: 왼팔(i>=8)은 모델 출력 0-7 사용
                    if arm_swap and i >= 8:
                        action_idx = i - 8
                        stats_idx = i - 8
                    else:
                    ...
                    normalized_val = action_values[action_idx]

                    if norm_type == 'MEAN_STD' and hasattr(self, 'action_mean') and hasattr(self, 'action_std'):
                        std = self.action_std[stats_idx]
                        if std > 1e-6:
                            linear = normalized_val * std + self.action_mean[stats_idx]
                        else:
                            linear = self.action_mean[stats_idx]
                    else:
                        range_val = self.action_q99[stats_idx] - self.action_q01[stats_idx]
                        linear = (normalized_val + 1.0) * range_val / 2.0 + self.action_q01[stats_idx]
    ...
    def apply_action(self, action, debug_log=False):
        """액션 적용 (레거시 - 텐서 입력)"""
        try:
            if action is None:
                return
            ...
            if self._debug_count < 5:
                self._debug_count += 1
                self.log(f"[DEBUG] arm_swap={arm_swap}, norm={norm_type}")
                self.log(f"[DEBUG] Model output[0:8]: {[f'{v:.3f}' for v in action_values[:8]]}")
                if hasattr(self, 'action_mean'):
                    self.log(f"[DEBUG] action_mean[0:8]: {[f'{v:.3f}' for v in self.action_mean[:8]]}")
                    self.log(f"[DEBUG] action_std[0:8]: {[f'{v:.3f}' for v in self.action_std[:8]]}")

            # 디버그: 정규화된 액션값 (조건부)
            if debug_log:
                self.log(f"[DEBUG] Action {norm_type} (R arm): {[f'{v:.3f}' for v in action_values[:8]]}")
            ...
            for i, joint_id in enumerate(self.joint_ids):
                if i < len(action_values):
                    # arm_swap_mode: 왼팔(i>=8)은 모델 출력 0-7 사용
                    if arm_swap and i >= 8:
                        action_idx = i - 8  # 왼팔 → 모델 indices 0-7
                        stats_idx = i - 8   # stats도 0-7 사용
                    else:
                        action_idx = i
                        stats_idx = i
                    ...
                    if norm_type == 'MEAN_STD' and hasattr(self, 'action_mean') and hasattr(self, 'action_std'):
                        # MEAN_STD 역정규화: x = normalized * std + mean
                        std = self.action_std[stats_idx]
                        if std > 1e-6:
                            linear = normalized_val * std + self.action_mean[stats_idx]
                        else:
            ...
            if debug_log:
                ...
                # 상세 관절 디버그 로그 저장 (파일에 자동 기록)
                if hasattr(self, '_last_current_positions') and hasattr(self, '_last_state_normalized'):
                    self._write_joint_debug(
                        frame_num=self._debug_frame_count,
                        current_positions=self._last_current_positions,
                        state_normalized=self._last_state_normalized,
                        action_output=action_values,
                        action_positions=positions
                    )

            # 디버그: 왼팔 위치 (처음 5번만)
            if hasattr(self, '_debug_count') and self._debug_count <= 5:
                self.log(f"[DEBUG] Linear[8:16] (L arm): {[f'{v:.3f}' for v in linear_values[8:16] if len(linear_values) > 8]}")
                self.log(f"[DEBUG] Positions[8:16] (L arm): {positions[8:16] if len(positions) > 8 else positions}")
